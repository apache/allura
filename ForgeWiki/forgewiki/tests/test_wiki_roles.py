...
def test_role_assignments():
    ...
    assert c.app.config.acl['create'] == [role_developer]
    assert c.app.config.acl['edit'] == [role_developer]
    assert c.app.config.acl['delete'] == [role_developer]
    assert c.app.config.acl['edit_page_permissions'] == c.project.acl['plugin']
