...
def test_wiki2markdown_talk():
    ...
    cmd.run([test_config, '--extract-only', '--output-dir', output_dir, 'talk'])
    cmd.command()
    ...
    for p in projects:
        ...
        pid = "%s" % p._id
        file_path = os.path.join(output_dir, pid, "talk.json")
        assert os.path.isfile(file_path)

    cmd = wiki2markdown.Wiki2MarkDownCommand('wiki2markdown')
    cmd.run([test_config, '--load-only', '--output-dir', '/tmp/wiki2markdown', 'talk'])
    cmd.command()
    for p in M.Post.query.find().all():
