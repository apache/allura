...
class BulkExport(object):
    ...
    def process(self, project, tools, user, filename=None, send_email=True):
        ...
        email = {
            'toaddr': str(user._id),
            'fromaddr': str(tg.config['forgemail.return_path']),
            'sender': str(tg.config['forgemail.return_path']),
            'reply_to': str(tg.config['forgemail.return_path']),
            'message_id': h.gen_message_id(),
            'subject': 'Bulk export for project %s completed' % project.shortname,
