...
class RemoveDuplicateTroves(ScriptTask):
    ...
    def execute(cls, options):
        ...
        log.info('Found %s duplicate categories: %s', len(duplicates), list(duplicates.keys()))
        for name, dups in duplicates.items():
            ...
            for _id, ps in projects_with_category.items():
                log.info('  with id %s: %s', _id, [p.shortname for p in ps])
            priority = [(i, len(ps)) for i, ps in list(projects_with_category.items())]
