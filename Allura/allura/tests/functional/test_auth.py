...
class TestAuth(TestController):
    ...
    def test_auth_redirect_attack(self):
        payloads = [
            "https://evil.com", "http://evil.com",  # standard bad
            "//evil.com", "///evil.com", "////evil.com",  # no protocol
            "http:evil.com", "http:///evil.com", "http:////evil.com",  # http non-standard # of slashes
            "https:evil.com", "https:///evil.com", "https:////evil.com", # https non-standard # of slashes
            # browsers treat "\\" as "/" and strip tab/CR/LF before parsing, so these reach evil.com too
            "/\\evil.com", "\\\\evil.com", "http:/\\evil.com",
            "/\t/evil.com", "http:/\t/evil.com", " //evil.com",
            # look like our own host, but aren't
            "http://localhost@evil.com/", "http://localhost.evil.com/",
            "https://localhost/p/test/",  # wrong protocol even rejected
        ]
        for p in payloads:
            r = self.app.get("/auth/", extra_environ={"username": "test-admin"}, params={"return_to": p}, status=302)
            loc = r.headers["Location"]
            assert loc == 'http://localhost/', f"payload={p!r} Location={loc!r}"

    def test_auth_redirect_ok(self):
        payloads = [
            "/p/test/",
        ...
        for p in payloads:
            r = self.app.get("/auth/", extra_environ={"username": "test-admin"}, params={"return_to": p}, status=302)
            loc = r.headers["Location"]
            assert loc == 'http://localhost/p/test/', f"payload={p!r} Location={loc!r}"
...
class TestPasswordExpire(TestController):
    ...
    def test_return_to(self):
        ...
        self.set_expire_for_user()
        with h.push_config(config, **{'auth.pwdexpire.days': 90}):
            r = login(self.app, query_string='?' + urlencode({'return_to': return_to}))
        ...
        with h.push_config(config, **{'auth.multifactor.totp': 'false'}):
            ...
            for url in ['/auth/preferences/totp_new',
                ...
                self.app.post(url,
                              ...
                              status=404)
            self.app.get('/auth/multifactor/', status=404,
                         extra_environ={'username': '*anonymous'},)
...
class TestEmailAuthCode(TestController):
    ...
    @mock.patch.dict(config, {'auth.email_auth_code.enabled': True})
    @patch('allura.tasks.mail_tasks.send_system_mail_to_user')
    def test_email_auth_code(self, send_system_mail_to_user):
        self.app.extra_environ = {'disable_auth_magic': 'True'}
        user = M.User.by_username('test-user')
        ...
        with patch('allura.lib.plugin.AuthenticationProvider.trusted_login_source', return_value=False):
            r = self.app.get('/auth/?return_to=/p/test/')
            f = r.forms[0]
            encoded = self.app.antispam_field_names(f)
            f[encoded['username']] = 'test-user'
            f[encoded['password']] = 'foo'
            ...
            # Validate that the user is directed to the "check your email" page
            assert 'Check Your Email' in r.text
            assert r.request.path == '/auth/login_email_sent'

            user = M.User.by_username('test-user')
            user_token = user.get_tool_data('AuthEmailCode', 'token')
            user_expiry_ts = user.get_tool_data('AuthEmailCode', 'token_expiry')

            # Make sure a token was generated
            assert user_token
            ...
            link = [line for line in email_text.splitlines() if '/auth/login_email_verify' in line][0]
            assert f'token={user_token}' in link

            # Click the verification link and validate the user is logged in and at final return_to URL
            r = self.app.get(link, status=302)
            assert r.location == 'http://localhost/p/test/'
            assert r.session.get('username') == 'test-user'
    ...
    @mock.patch.dict(config, {'auth.email_auth_code.enabled': True})
    def test_input_incorrect_auth_token(self):
        self.app.extra_environ = {'disable_auth_magic': 'True'}
        user = M.User.by_username('test-user')
        user.set_pref('multifactor', False)
    ...
    @mock.patch.dict(config, {'auth.email_auth_code.enabled': True})
    def test_input_expired_auth_token(self):
        self.app.extra_environ = {'disable_auth_magic': 'True'}
        user = M.User.by_username('test-user')
        user.set_pref('multifactor', False)
        ...
        with patch('allura.lib.plugin.AuthenticationProvider.trusted_login_source', return_value=False):
            ...
            # Set the token expiration to 15 minutes ago
            user = M.User.by_username('test-user')
            user_token = user.get_tool_data('AuthEmailCode', 'token')
