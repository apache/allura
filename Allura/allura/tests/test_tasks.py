...
class TestMailTasks:
    ...
    @mock.patch.dict(tg.config, {
        ...
        'forgemail.sender_authentication.header': 'X-SourceForge-Sender-Auth',
    })
    def test_route_email_enabled_uses_authenticated_from_user(self):
        message = dedent('''\
            From: "Test Admin" <test-admin@users.localhost>
            X-SourceForge-Sender-Auth: v=1; dmarc=pass;
    ...
    def test_route_email_enabled_does_not_trust_spoofed_envelope(self):
        message = dedent('''\
            From: "Test Admin" <test-admin@users.localhost>
            X-SourceForge-Sender-Auth: v=1; dmarc=fail;
             from-domain=users.localhost
