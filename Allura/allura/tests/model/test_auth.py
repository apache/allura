...
class TestAuth:
    ...
    def test_display_name_is_stored_encrypted(self):
        ...
        assert user.display_name_encrypted == M.User.encr('Encrypted Display Name')
    ...
    def test_preference_email_address_is_stored_encrypted_on_creation(self):
        user = M.User(
        ...
        assert 'email_address' not in raw_preferences
    ...
    def test_preference_email_address_updates_encrypted_field(self):
        user = M.User(
    ...
    def test_get_tool_data_can_decrypt_encrypted_value(self):
        user = M.User(
        ...
        assert user.get_tool_data('test_tool', 'field') == 'old field'
    ...
    def test_personal_data_fields_stay_encrypted_on_update(self):
        user = M.User(
            ...
            last_access={
                'login_ip': '192.0.2.2',
                'session_ip': '198.51.100.2',
            },
            socialnetworks=[
                {'socialnetwork': 'Mastodon', 'accounturl': 'https://old.example/@user'},
                {'socialnetwork': 'Other', 'accounturl': 'https://remove.example/user'},
            ],
            telnumbers=['+1-555-0101', '+1-555-0102'],
        )
        ThreadLocalODMSession.flush_all()

        assert user.telnumbers_encrypted == [
            M.User.encr('+1-555-0101'),
            M.User.encr('+1-555-0102'),
        ]
        user.last_access['login_ip'] = '203.0.113.2'
        user.last_access['session_ip'] = None
        user.socialnetworks[0]['accounturl'] = 'https://new.example/@user'
        user.socialnetworks.pop()
        user.telnumbers.remove('+1-555-0101')
    ...
    def test_user_login_details_ip_is_stored_encrypted(self):
        detail = M.UserLoginDetails(
            user_id=c.user._id,
            ip='192.0.2.3',
            ua='PersonalDataEncryptionTest/1.0',
        )
    ...
    def test_set_display_name_pref_updates_encrypted_field_and_cache(self):
        ...
        assert user.display_name_encrypted == M.User.encr('Updated Display Name')
