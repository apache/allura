...
class ProjectAdminController(BaseController):
    ...
    def join_neighborhood(self, nid):
        ...
        if not nid:
            ...
            request.session.flash('Joined %s' % n.name)
            redirect(c.project.url() + 'admin/')
        nid = ObjectId(str(nid))
        if nid not in c.project.neighborhood_invitations:
            request.session.flash('No invitation to that neighborhood', 'error')
            redirect('.')
    ...
    def update_mounts(self, subproject=None, tool=None, new=None, **kw):
        ...
        try:
            ...
            request.session.flash('%s: %s' % (exc.__class__.__name__, exc.args[0]),
                  'error')
        g.post_event('project_updated')
        redirect('tools')
...
class GroupsController(BaseController):
    ...
    def update(self, card=None, **kw):
        for pr in card:
            ...
            for username in new_users:
                ...
                if not user:
                    request.session.flash('User %s not found' % pr['new'], 'error')
                    redirect('.')
    ...
    def create(self, name=None, **kw):
        if M.ProjectRole.by_name(name):
            request.session.flash('%s already exists' % name, 'error')
        else:
...
class GroupController(BaseController):
    ...
    def update(self, _id=None, delete=None, name=None, **kw):
        ...
        if delete:
            _id.delete()
            request.session.flash('%s deleted' % name)
            redirect('..')
        _id.name = name
        request.session.flash('%s updated' % name)
        redirect('..')
