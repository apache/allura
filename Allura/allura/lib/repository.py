...
class RepoAdminController(DefaultAdminController):
    ...
    def set_default_branch_name(self, branch_name=None, **kw):
        if (request.method == 'POST') and branch_name:
            ...
            project_url = self.repo.default_url_path(c.project, self.app.config.tool_name)
    ...
    def set_checkout_url(self, **post_data):
        ...
        if 'external_checkout_url' not in request.validation.errors:
            ...
            flash_msgs.append(f"Invalid external checkout URL: {request.validation.errors['external_checkout_url']}.")
