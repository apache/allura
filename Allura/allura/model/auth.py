...
class User(MappedClass, ActivityNode, ActivityObject, SearchIndexable):
    ...
    class __mongometa__:
        ...
        extensions = [UserFieldEncryptionMapperExtension]
        custom_indexes = [
    ...
    email_addresses = DecryptedListProperty('email_addresses_encrypted')
    ...
    preferences = NestedEncryptedProperty(dict(
    ...
    def make_password_reset_url(self):
        hash = h.nonce(42)
        expiry_delta = timedelta(seconds=int(config.get('auth.recovery_hash_expiry_period', 600)))
